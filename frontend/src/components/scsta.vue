<template>
    <el-card>
        ...
        <!--        数据列表-->
        <el-table :data="scdata" border style="width: 100%" highlight-current-row @cell-dblclick="editCell">
...
<script>
    ...
    export default {
        ...
        methods: {
            // 检索，待细化
            async search() {
                var res = await axios.get('/api/home/processscprepare/', {
                    params: {
            ...
            getscdataPage(page){
                axios.get('/api/home/processscprepare/',{params:{
                    page:page
                    }}).then(res=>{
                    this.scdata = res.data.results;
                })
            ...
            CurrentChange(currentPage){
                this.getscdataPage(currentPage);
            },
            // 页码变化
            currentChange(page) {
                this.currentPage = page;
                this.getscdataPage(page);
            },
            goback(){
                this.getscdataPage(this.currentPage)
            },
            //双击编辑备注内容
            editCell(row, col) {
