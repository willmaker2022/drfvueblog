<template>
    <el-container style="background: white">
        <el-header style="padding-bottom: 0px;padding-top: 20px; padding-left: 20px">
            <!--    搜索框和按钮-->
<!--            <el-row :gutter="12" margin-bottom=20px style="position: fixed">-->
                <el-col :span="6">
                    <el-input
                            v-model="searchInfo"
                            placeholder="输入检索的内容"
...
<!--            </el-row>-->
        ...
        <el-main>
            <!--        数据列表-->
            <el-table :data="asdata" border style="width: 100%"
                      highlight-current-row @cell-dblclick="editCell">
...
<script>
    ...
    export default {
        ...
        methods: {
            // 检索，待细化
            async search() {
                var res = await axios.get('/api/home/processassemble/', {
                    params: {
                        search: this.searchInfo
                    }
                })
                if (res.data.count !== 0) {
                    this.asdata = res.data.results
                } else {
                    this.asdata = []
                }
            },
            ...
            getasdata() {
                axios.get('/api/home/processassemble/').then(res => {
                    this.asdata = res.data.results;
            ...
            getasdataPage(page) {
                axios.get('/api/home/processassemble/', {
                    params: {
                        page: page
                    }
                }).then(res => {
            ...
            CurrentChange(currentPage) {
                this.getasdataPage(currentPage);
            },
            // 页码变化
            currentChange(page) {
                this.currentPage = page;
                this.getasdataPage(page);
            },
            goback() {
                this.getasdataPage(this.currentPage)
            },
            //双击编辑备注内容
            editCell(row, col) {
